...
int main(int argc, char *argv[]) {  
    ...
    pthread_create(&t_receiver,NULL,(void *)receive_mq,NULL); //dispara receptor
...
void *receive_mq(void *param) {
    ...
    if ((cola = mq_open("/M1", O_RDONLY)) == -1) {
        perror("No se puede acceder a la cola de mensajes");
        pthread_exit(0);
    }

    // Recibe el mensaje
    if (mq_receive(cola, buff, sizeof(buff), &prio) == -1) {
        perror("Error al recibir el mensaje");
        pthread_exit(0);
    }

    printf("Se recibió el mensaje [%s] con prioridad %d\n", buff, prio);
